...
class _RegistrationPageState extends State<RegistrationPage> {
  ...
  // --- ВАЛИДАТОРЫ ---
  String? _validateName(String? value) => (value == null || value.trim().isEmpty) ? 'Name cannot be empty' : null;
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          suffixIcon: IconButton(onPressed: onDelete, icon: const Icon(Icons.delete_outline, color: Colors.deepOrangeAccent)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Register Form', style: TextStyle(fontWeight: FontWeight.bold))),
...
// ЭТОТ КЛАСС ДОЛЖЕН БЫТЬ ВНЕ _RegistrationPageState
class UserInfoPage extends StatelessWidget {
  final String name, phone, email, lifeStory;
  const UserInfoPage({super.key, required this.name, required this.phone, required this.email, required this.lifeStory});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('UserInfoPage')),
      body: Padding(
        padding: const EdgeInsets.all(20),
        child: Column(
          children: [
            Text('Full Name: $name', style: const TextStyle(fontSize: 18)),
            Text('Phone: $phone', style: const TextStyle(fontSize: 18)),
            Text('Email: $email', style: const TextStyle(fontSize: 18)),
            Text('Story: $lifeStory', style: const TextStyle(fontSize: 18)),
          ],
        ),
      ),
    );
